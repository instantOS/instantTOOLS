...
checkaur() {
	# grep -q breaks this for some reason.
	# I don't know why, don't touch if you don't know either
	if curl -s "https://aur.archlinux.org/packages/$1" | grep 'Git Clone URL' &>/dev/null; then
		return 0
	fi
	return 1
}
